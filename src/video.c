...
// TODO bring this stuff back when ready
...
uint8_t video_read(uint8_t reg, bool debugOn) {
	...
	switch (reg & 0x1F) {
      ...
      get_and_inc_address(reg - 3, false);
...
void video_write(uint8_t reg, uint8_t value) {
	...
	switch (reg & 0x1F) {
		...
		case 0x1B: break; //audio_render(); pcm_write_ctrl(value); break;
		case 0x1C: break; //audio_render(); pcm_write_rate(value); break;
		case 0x1D: break; //audio_render(); pcm_write_fifo(value); break;
